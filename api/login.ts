...
export default async function handler(req: any, res: any) {
  try {
    ...
    if (role === "admin") {
      const { data: admin, error } = await supabase.from("admins").select("id,name,email,password_hash").eq("email", String(email).trim().toLowerCase()).maybeSingle();
      if (error || !admin) return res.status(401).json({ error: "Invalid admin credentials" });
      let ok = false;
      if (admin.password_hash) {
        try { ok = await bcrypt.compare(password, admin.password_hash); } catch { ok = false; }
      }
    ...
    if (role === "lecturer") {
      const { data: lec, error } = await supabase.from("lecturers").select("id,name,email,password_hash,password").eq("email", String(email).trim().toLowerCase()).maybeSingle();
    ...
    if (role === "student") {
      ...
      const { data: stu, error } = await supabase.from("students").select("id,name,email,registration_number,batch,specialization,password_hash,password").eq("registration_number", regNum).maybeSingle();
