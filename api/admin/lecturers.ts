...
export default async function handler(req: any, res: any) {
  try {
    ...
    if (req.method === "POST") {
      ...
      if (!name || !email || !password) return res.status(400).json({ error: "Lecturer name, email, and password are required." });
      if (password.length < 6) return res.status(400).json({ error: "Password must be at least 6 characters long." });
      const normalizedEmail = String(email).trim().toLowerCase();
      const { data: existing } = await supabase.from("lecturers").select("id").eq("email", normalizedEmail).maybeSingle();
  ...
  } catch (err: any) { console.error("[admin/lecturers]", err); return res.status(500).json({ error: err?.message || "Internal server error." }); }
}
