...
export function readAuthContext(req: any): AuthContext | null {
  ...
  try {
    ...
    return {
      role: payload.role as Role, userId: payload.id, email: payload.email, name: payload.name,
      batch: payload.batch, specialization: payload.specialization, registrationNumber: payload.registrationNumber,
    };
  } catch { return null; }
}
...
// Re-export crypto for legacy password migration
export { crypto };
